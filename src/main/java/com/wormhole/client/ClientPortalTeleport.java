 ...
 * EVERY portal volume. This replaces the old per-frame plane-crossing + grace-frame heuristic, and is
 * what makes the depth-negating transform safe (you land on the mirror side, still inside the
 * destination volume, and simply walk out).
 */
public final class ClientPortalTeleport {
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        List<PortalPair> pairs = PortalRegistry.clientPairs();
        if (pairs.isEmpty()) {
            justTeleported = false;
            return;
        }
        ...
        if (justTeleported) {
            // Hold off until we have physically left every portal volume before allowing another cross.
