...
uniform sampler2D Src0; // camera-positioned SOURCE cube (own universe) for around-rays
...
uniform sampler2D AroundLut;     // around-branch bend delta(b), normalized by DELTA_MAX
...
vec3 sampleSrc(vec3 d) {
    ...
    float v = dot(d, Up[best].xyz) / bestDot;
    vec2 uv = vec2(u * 0.5 + 0.5, v * 0.5 + 0.5);
    if (best == 0) return texture(Src0, uv).rgb;
    else if (best == 1) return texture(Src1, uv).rgb;
    else if (best == 2) return texture(Src2, uv).rgb;
    else if (best == 3) return texture(Src3, uv).rgb;
    else if (best == 4) return texture(Src4, uv).rgb;
    return texture(Src5, uv).rgb;
}
...
void main() {
    ...
    vec3 col;
    if (alphaRatio < 1.0) {
        // THROUGH branch: passes to the other universe -> partner cube, bent by the real geodesic.
        ...
        // AROUND branch: stays in this universe, bent radially toward the throat -> source cube.
        float q = alphaRatio; // angular radius (1 at the silhouette)
        float bend = 0.0;
        if (q <= B_MAX) {
            float bu = (q - 1.0) / (B_MAX - 1.0);
            bend = min(texture(AroundLut, vec2(bu, 0.5)).r * DELTA_MAX, DELTA_MAX);
        }
